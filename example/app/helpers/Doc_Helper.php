...
class Doc_Helper {
     ...
     *
     * @param $container_upload_input_name - Name of the input used to upload the container.
     *
    ...
    public static function get_encoded_hashcode_version_of_container ($container_upload_input_name) {
        ...
        $doc = $hashcode_session->containerFromString(
            file_get_contents($_FILES[$container_upload_input_name]['tmp_name'])
        );
    ...
    public static function create_container_with_files ($container_data, $datafiles) {
        $path_to_created_doc = File_Helper::get_upload_directory() . DIRECTORY_SEPARATOR . get_original_container_name();
        $hashcode_session = self::get_hashcode_session();
        ...
        File_Helper::delete_if_exists($path_to_created_doc);
    ...
    public static function add_datafile_via_dds ($path_to_datafile, $datafile_mime_type) {
        $container_type = Doc_Helper::get_container_type(get_original_container_name());
        ...
        $digest_value = $container_type === 'BDOC'
            ? BdocContainer::datafileHashcode($data)
            :
            DdocContainer::datafileHashcode($filename, $datafile_id, $datafile_mime_type, $data);
    ...
    public static function get_container_type ($filename) {
        $extension = File_Helper::parse_file_extension($filename);
    ...
    public static function get_next_datafile_id () {
        if (!file_exists(
            File_Helper::get_upload_directory() . DIRECTORY_SEPARATOR . get_original_container_name()
        )
        ) {
            return 'D0';
        }
     ...
     * applications index.
     *
     * @param $container_type_input_name - Name of the selectbox where container type is selected.
     *
    ...
    public static function get_desired_container_type ($container_type_input_name) {
        if (!isset($_POST[$container_type_input_name])) {
            throw new Exception('Invalid container type.');
        }
        $valid_container_types = array ('BDOC 2.1', 'DIGIDOC-XML 1.3');
        $chosen_container_type = $_POST[$container_type_input_name];
        if (!in_array($chosen_container_type, $valid_container_types)) {
            throw new Exception("Invalid container type '$chosen_container_type'.");
        }
        $parts = explode(' ', $chosen_container_type);
        $short_type = 'BDOC';
